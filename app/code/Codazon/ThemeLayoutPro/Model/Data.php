...
class Data extends \Magento\Framework\Model\AbstractModel
{
    ...
    public function exportPatch($patchList, $zipFileName = 'patch-01.zip')
    {
        $list = $this->getPatchList($patchList);
        echo "<textarea style='height: 2000px; resize: none; width: 100%; background: #fff;' disabled>";
        if (!empty($list['files'])) {
            echo "<ul>\n";
    ...
    public function buildProjectAssets($productPath, $exportToDefault = true)
    {
        ...
        if ($type == 'header') {
            ...
            $collection = $this->mainContentModel->getCollection()->setStoreId(0)->addAttributeToSelect(['themelayout_title']);
        } elseif ($type == 'footer') {
    ...
    public function buildAssets($onlyMainContent = false, $exportToDefault = true)
    {
        $result = [];
